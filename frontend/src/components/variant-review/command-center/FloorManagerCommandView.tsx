...
import { CommandCenterActionForm, hasSupportedAction } from './CommandCenterActionForm'
import { formatBlockedReason } from './AssemblerCommandView'
...
export function FloorManagerCommandView({ vm }: Props) {
  ...
  return (
    <div className="p-4 space-y-4">
      {blockedUnits.length > 0 && (
        <div className="space-y-2">
          <div className="text-xs font-semibold uppercase tracking-widest t-on-error">
            Attention — {blockedUnits.length} need{blockedUnits.length === 1 ? 's' : ''} resolution
          </div>
          {blockedUnits.map((u) => (
      ...
      <div className="rounded-lg border surf-container b-outline-var px-3 py-2">
        <div className="text-[11px] font-semibold uppercase tracking-widest t-on-surface-var mb-1">
          Secondary Context
        </div>
        <p className="text-xs t-on-surface-var">
